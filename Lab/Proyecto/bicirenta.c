#include <stdio.h> //Declaramos la libreria de entradas y salidas.
#include <time.h> //Declaramos time.h para poder obtener la hora del sistema.
#include <string.h> //Declaramos string.h para manejar cadenas.
#include <stdlib.h> //Declaramos stdlib.h para la presentación del sistema (sleeps y clears).

typedef struct Def_Biciestacion //Estructura de la Biciestacion
{
...
typedef struct Def_Bicicleta //Estructura de la Bicicleta
{
...
typedef struct Def_Usuario //Estructura del Usuario
{
...
void ListaActivos(TipoUsuario *PrimerUsuario); //Esta función recorre la lista dinámica de Usuarios e imprime los datos de aquellos que se encuentren activos.
void ListaBiciestacionesActivas(TipoBiciestacion *PrimeraBiciestacion); //Esta función recorre la lista dinámica de Biciestaciones e imprime los datos de aquellas que se encuentren activas.
...
void MenuAdmin (TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, int Numeracion [3], int ID_Sesion); //Este menú imprime el menu del administrador y lleva al mismo a sus respectivas herramientas.
void RentarBicicleta(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, TipoBiciestacion NuevaBiciestacion, int ID_Usuario, int *ErrorRentar); //Esta función comprueba si el usuario puede obtener una bicicleta de la biciestación especificada, en ese caso, a la bicicleta se le asigna un timestamp, a la biciestacion se le suma un espacio vacio y resta un espacio lleno y al usuario se le coloca un Flag que indica que tiene una bicicleta.
void EstacionarBicicleta(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario,TipoBiciestacion NuevaBiciestacion, int ID_Usuario, int *ErrorEstacionar); //Esta funcion verifica si el usuario puede estacionar una bicicleta en la biciestación que especificó, en ese caso, a la bicicleta le retira el timestamp, a la biciestacion le suma un espacio lleno y resta un espacio vacio y al usuario le quita la flag que indica que tiene una Bicicleta
void MostrarSaldo(TipoUsuario *PrimerUsuario, int ID_Sesion); //Esta función recibe el ID del usuario que está usando el sistema, lo busca en la lista dinámica e imprime la cantidad de multas que tiene.
void CompararHoras(TipoUsuario *PrimerUsuario, TipoBicicleta *PrimeraBicicleta); //Esta función se encarga de determinar si una bicicleta fue o no entregada a tiempo.
void BorrarListaBiciestacion(TipoBiciestacion *PrimeraBiciestacion); //Esta función limpia de la memoria la lista dinámica de las Biciestaciones.
void BorrarListaBicicleta(TipoBicicleta *PrimeraBicicleta); //Esta función limpia de la memoria la lista dinámica de las Bicicletas.
void BorrarListaUsuario(TipoUsuario *PrimerUsuario); //Esta función limpia de la memoria la lista dinámica de los Usuarios.
void AltaBiciestaciones(TipoBiciestacion **PrimeraBiciestacion, TipoBiciestacion NuevaBiciestacion, int Numeracion [3], int ID_Sesion); //Esta función inserta al final de la lista dinámica de Biciestaciones la biciestación que se leyó en "MenuAdmin".
void BajaBiciestaciones (TipoBiciestacion **PrimeraBiciestacion, int IDBaja, int *Encontro, int ID_Sesion); //Esta función busca la biciestación que se haya especificado en "MenuAdmin" y la marca como Inactiva.
void AltaBicicletas(TipoBicicleta **PrimeraBicicleta,TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta NuevaBicicleta, int Numeracion [3], int *Verificado, int ID_Sesion); //Esta función inserta al final de la lista dinámica de Bicicletas la bicicleta que se leyó en "MenuAdmin".
void BajaBicicletas (TipoBicicleta **PrimeraBicicleta, int IDBaja, int *Encontro, int ID_Sesion); //Esta función busca la bicicleta que se haya especificado en "MenuAdmin" y la marca como Inactiva.
void ReasignarBicicletas(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta,TipoBicicleta NuevaBicicleta, TipoBiciestacion NuevaBiciestacion, int *Verificar_Reasignacion, int ID_Sesion); //Esta función verifica si la Bicicleta que se especificó en "MenuAdmin" se puede mover a la Biciestación que se especificó en "MenuAdmin", en ese caso, se resta un lugar vacio a la Biciestación de destino, un lugar lleno a la Biciestación de origen, se suma un lugar lleno a la Biciestación de destino, un lugar vacío a la Biciestación de origen y se le cambia el ID de Biciestación a la Bicicleta.
void AltaUsuario(TipoUsuario **PrimerUsuario, TipoUsuario NuevoUsuario, int Numeracion [3], int ID_Sesion); //Esta función inserta al final de la lista dinámica de Usuarios al usuario que haya leído.
...
void BajaUsuario (TipoUsuario **PrimerUsuario, int IDBaja, int *Encontro, int ID_Sesion); //Esta función busca al Usuario que se haya especificado en "MenuAdmin" y lo marca como Inactivo.
void CargarBase (int Numeracion [3], int *PrimeraEjecucion, TipoBiciestacion **PrimeraBiciestacion, TipoBicicleta **PrimeraBicicleta, TipoUsuario **PrimerUsuario); //Esta función verifíca si los archivos necesarios para la ejecución están completos, en ese caso, carga sus contenidos a las listas dinámicas, de lo contrario, crea los archivos, inicializa las listas dinámicas e indica a la función principal que esta es la primera ejecución del programa.
void GuardarDatos(TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, int Numeracion[3]); //Esta función deposita el contenido de las listas dinámicas en distintos archivos para conservarlas y sale del programa.
...
void MenuAdmin (TipoBiciestacion *PrimeraBiciestacion, TipoBicicleta *PrimeraBicicleta, TipoUsuario *PrimerUsuario, int Numeracion [3], int ID_Sesion) //Este menú imprime el menu del administrador y lleva al mismo a sus respectivas herramientas.
{
  TipoBiciestacion NuevaBiciestacion; //Temporal para guardar una Biciestación que se vaya a dar de alta.
  TipoUsuario NuevoUsuario; //Temporal para guardar un Usuario que se vaya a dar de alta.
  TipoBicicleta NuevaBicicleta; //Temporal para guardar una Bicicleta que se vaya a dar de alta.
  int Seleccion, Repetir, IDBaja, Encontro, ErrorLectura, VerificarID_Biciestacion, Verificar_Reasignacion, InformacionAdicional, InformacionAdicional2; //Seleccion guarda la opción del menú seleccionada, Repetir se usa para el Logout, IDBaja es una temporal donde se guarda el ID del elemento que se va a dar de baja (ya sea Biciestacion, Bicicleta o Usuario), Encontro, ErrorLectura, VerificarID_Biciestacion y Verificar_Reasignación son Flags para comprobar errores, InformacionAdicional e InformaciónAdicional2 son temporales donde se guardan los IDs necesarios para llenar la bitacora.
  char CodigoDeAccion [200]; //Temporal donde se guarda el código de acción de cada función para ponerlo en la bitácora.
  Repetir = 1; //Inicializa repetir en 1 para que el Menú del administrador se repita hasta que se escoja la opción de Logout (donde repetir se iguala a 0).
  
  char Enter; //Variable que no se procesa, sirve para la funcionalidad "Ingrese cualquier caracter para continuar / reintentar...".
  do {
    Encabezado (); //Imprimimos el encabezado
    system ("sleep 0.04"); //Se imprimen las opciones disponibles, los "sleep" son para presentación.
    ...
    printf ("Su selección: ");
    __fpurge (stdin);
    scanf (" %d", &Seleccion); //Leemos la opción a la que quiere ir el administrador.
    switch (Seleccion) //En base a la selección se ejecutarán diferentes funciones.
      {
      case 1:
	Encabezado ();
	ErrorLectura = 0;
	...
	printf ("Escribe el nombre de la biciestacion: "); //Solicitamos los datos para una nueva biciestación
	if ((scanf(" %[^\n]",NuevaBiciestacion.Nombre)) == 0) //Se lee la información, los "if (scanf)" son para comprobar que los datos se están leyendo correctamente.
	  ErrorLectura = 1;
	...
	if ((scanf(" %d", &NuevaBiciestacion.LugaresDisponibles)) == 0)
	  ErrorLectura = 1;
	printf("\n");
	if (ErrorLectura == 0) //Si no hubo error, se le pide al administrador que confirme el guardado de datos.
	  {
	    ...
	    __fpurge (stdin);
	    scanf (" %c", &Enter);
	    AltaBiciestaciones(&PrimeraBiciestacion,NuevaBiciestacion,Numeracion, ID_Sesion); //Se da de alta la biciestación
	  }
	else
	  {
...
void GuardarEnBitacora (int ID_Sesion, char CodigoDeAccion [200], int InformacionAdicional, int InformacionAdicional2) //Esta función actualiza la bitacora cada un administrador: Inicia / Cierra sesión, Da de Alta / Baja: Biciestaciones, Bicicletas y Usuarios y Reasigna una bicicleta.
{
  ...
  if (InformacionAdicional2 == 0) //Si se obtuvo un segundo ID
    {
      if ((strcmp ("Login", CodigoDeAccion)) == 0 || (strcmp ("Logout", CodigoDeAccion)) == 0) //Si fue un login o un logout
	{
	  fprintf (ArchivoBitacora, "%s %s %d\n", HoraSistema, CodigoDeAccion, ID_Sesion); //Se guarda en la bitacora la Timestamp, el código de acción y el ID del administrador que se logueó / deslogueó
	}
      else //Si no fue un login
	{
	  fprintf (ArchivoBitacora, "%s %s %d\n", HoraSistema, CodigoDeAccion, InformacionAdicional); //Se guarda en la bitacora la Timestamp, el código de acción y el ID que corresponde
	}
    }
  else
    {
      fprintf (ArchivoBitacora, "%s %s %d %d\n", HoraSistema, CodigoDeAccion, InformacionAdicional, InformacionAdicional2); //Se guarda en la bitacora la Timestamp, el código de acción y los 2 IDs correspondientes.
    }
